...
def main():
    ...
    bigquery_load_tasks = {
        config.INTERMEDIATE_OUTPUT_DIR: "bronze",
        config.SCHEMA_VERIF_OUTPUT_DIR: "bronze_schema_verification"
    }

    for i in bigquery_load_tasks.keys():
        bigquery_load.bronze_ingest_to_bigquery(
            data_dir=i,
            bigquery_schema_name=bigquery_load_tasks[i],
            logger=logger
        )
